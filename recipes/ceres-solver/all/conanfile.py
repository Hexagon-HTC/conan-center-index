...
class CeressolverConan(ConanFile):
    ...
    def validate(self):
        min_cppstd = "17" if Version(self.version) >= "2.2.0" else "14"
    ...
    def generate(self):
        ...
        tc.cache_variables["MINIGLOG"] = not self.options.use_glog
        tc.cache_variables["GFLAGS"] = False # useless for the lib itself, gflags is not a direct dependency
        tc.cache_variables["SUITESPARSE"] = False
        tc.cache_variables["LAPACK"] = False
        tc.cache_variables["SCHUR_SPECIALIZATIONS"] = self.options.use_schur_specializations
        tc.cache_variables["CUSTOM_BLAS"] = self.options.use_custom_blas
        tc.cache_variables["EIGENSPARSE"] = self.options.use_eigen_sparse
        ...
        if ceres_version >= "2.2.0":
            ...
            tc.cache_variables["CUDA"] = False
        if ceres_version >= "2.2.0":
            tc.cache_variables["EIGENMETIS"] = False
        if ceres_version >= "2.0.0":
            tc.cache_variables["PROVIDE_UNINSTALL_TARGET"] = False
            if is_apple_os(self):
                tc.cache_variables["ACCELERATESPARSE"] = True
        if ceres_version < "2.2.0":
            tc.cache_variables["CXSPARSE"] = False
            if is_msvc(self):
